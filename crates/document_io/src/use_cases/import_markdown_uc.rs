...
use common::parser_tools::content_parser::{ParsedElement, ParsedInline, format_runs_from_spans, parse_markdown};
    ...
    for (i, parsed_element) in parsed_elements.iter().enumerate() {
        ...
        match parsed_element {
            ...
            ParsedElement::Table(parsed_table) => {
                ...
                // elements linearly). The boundary comes from the same
                // positional flag every block uses — see the note on
