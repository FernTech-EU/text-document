...
pub enum FrameRelationshipField {
    Blocks,
    ParentFrame,
    Table,
}

impl Display for FrameRelationshipField {
...
pub struct FrameRepository<'a> {
    table: Box<dyn FrameTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> FrameRepository<'a> {
    pub fn new(table: Box<dyn FrameTable + 'a>, transaction: &'a Transaction) -> Self {
        FrameRepository { table, transaction }
    }
    ...
    pub fn create_orphan(
        ...
        entity: &Frame,
    ) -> Result<Frame, RepositoryError> {
        let new = self.table.create(entity)?;
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Frame(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Frame(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let blocks = entity.blocks.clone();

        // remove all strong relationships, initiating a cascade remove

        repository_factory::write::create_block_repository(self.transaction)?
            .remove_multi(event_buffer, &blocks)?;
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&DocumentRelationshipField::Frames, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Frame(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let mut blocks_ids: Vec<EntityId> = entities
            .iter()
            .flat_map(|entity| entity.as_ref().map(|entity| entity.blocks.clone()))
            .flatten()
            .collect();
        // remove duplicates
        blocks_ids.sort();
        ...
        repository_factory::write::create_block_repository(self.transaction)?
            .remove_multi(event_buffer, &blocks_ids)?;
        // Before removal, find which owner(s) reference these entities
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo =
                repository_factory::write::create_document_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&DocumentRelationshipField::Frames, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn set_relationship_multi(
        ...
        if !all_right_ids.is_empty() {
            match field {
                ...
                FrameRelationshipField::Table => {
                    let child_repo =
                        repository_factory::write::create_table_repository(self.transaction)?;
                    let found = child_repo.get_multi(&all_right_ids)?;
                    let missing: Vec<_> = all_right_ids
                        .iter()
                        .zip(found.iter())
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &DocumentRelationshipField::Frames)
    }
    ...
    pub fn restore(
...

        {
            ...
            emit(DirectAccessEntity::Block(EntityEvent::Updated), child_ids);
        }
...
pub struct FrameRepositoryRO<'a> {
    table: Box<dyn FrameTableRO + 'a>,
}
impl<'a> FrameRepositoryRO<'a> {
    pub fn new(table: Box<dyn FrameTableRO + 'a>) -> Self {
        FrameRepositoryRO { table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Frame>, RepositoryError> {
        self.table.get(id)
    }
