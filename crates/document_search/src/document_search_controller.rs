...
use crate::use_cases::addressable_text_uc::AddressableTextUseCase;
...
use common::event::DocumentSearchEvent::AddressableText;
...
// Hand-added (NOT generated by Qleany): see `addressable_text_uc`'s doc.
pub fn addressable_text(
    db_context: &DbContext,
    event_hub: &Arc<EventHub>,
) -> Result<AddressableTextResultDto> {
    // find_all's own unit of work, on purpose — the returned string is defined as "the
    // text find_all searches", so it is built by find_all's plumbing, not a copy of it.
    let uow_context = FindAllUnitOfWorkFactory::new(db_context);
    let mut uc = AddressableTextUseCase::new(Box::new(uow_context));
    let return_dto = uc.execute()?;
    event_hub.send_event(Event {
        origin: Origin::DocumentSearch(AddressableText),
        ids: vec![],
        data: None,
    });
    Ok(return_dto)
